...
(function (global, factory) {
  ...
  var transitionTicks = function transitionTicks(svg, data, opt, xAxis, yAxis) {
    var _transition = transition().duration(opt.animation.duration.update);
    var delay = function delay(d, i) {
      return i / data.length * opt.animation.duration.update;
    };

    _transition.select('.x.axis').delay(delay).call(xAxis);

    _transition.select('.y.axis').delay(delay).call(yAxis);
  ...
  var updateDimensionScale = function updateDimensionScale(data, opt) {
    ...
    switch (dim.type) {
      case Globals.DataType.DATE:
        ...
        var _range = extent$1(data, function (d) {
          return d[dim.accessor];
        });
      ...
      case Globals.DataType.NUMBER:
        ...
        var _rangeNm = extent$1(data, function (d) {
          return d[dim.accessor];
        });
    ...
    try {
      var _loop = function _loop() {
        ...
        var rangeList = yList.map(function (d) {
          return extent$1(data, function (datum) {
        ...
        var universalMax = max$1(rangeList, function (d) {
          return d[1];
        });
  ...
  var rescaleStackedMetric = function rescaleStackedMetric(nestedData, opt) {
    ...
    var _max = max$1(nestedData.map(function (d) {
      return max$1(d.values.map(function (e) {
        return e.y1;
      }));
    }));
  ...
  var animate = function animate(state) {
    ...
    var exitTransition = selection().transition().duration(_options.animation.duration.remove).each(function () {
  ...
  var limitSliceValues = function limitSliceValues(data, opt, color) {
    ...
    var total = sum$1(data, function (d) {
      return d.y;
    });
  ...
  var animateStates = function animateStates(initialState, finalState, duration, context, opt, dataRange) {
    if (opt.animation.enabled === false) {
      ...
      return new Promise(function (resolve, reject) {
        var interpolateParticles = array$2(initialState, finalState);
  ...
  var updateRadiusScale = function updateRadiusScale(state) {
    ...
    if (radiusDef.scale !== undefined && radiusDef.scale !== null && isFunction$2(radiusDef.scale)) {
      ...
      if (_options.data.r !== undefined && _options.data.r !== null && _options.data.r.accessor !== undefined && _options.data.r.accessor !== null) {
        var range = extent$1(_data, function (d) {
  ...
  var animate$5 = function animate(state) {
    ...
    var minY0 = min$1(data.nested.map(function (d) {
      return min$1(d.values.map(function (e) {
        return e.y0;
      }));
    }));
    ...
    animateStates$4(initialState, finalState, opt.animation.duration.update, frontContext, opt).then(function (res) {
      state._voronoi = applyVoronoi(frontContext, opt, res.reduce(function (acc, p) {
  ...
  var drawLabel = function drawLabel(context, opt, text, i, sliceNum, maxR, scale) {
    ...
    drawCircularText(context, text + '', 14, 'Oswald', opt.plots.axisLabelColor, opt.chart.width / 2, opt.chart.height / 2, radius, angle * i + angle / 2, 0);
  };
   ...
   * @returns {Function} Returns `func`.
   */
  var baseSetToString$1 = !defineProperty$2 ? identity$8 : function (func, string) {
    return defineProperty$2(func, 'toString', {
      'configurable': true,
      'enumerable': false,
      'value': constant$a(string),
      'writable': true
    });
  };
  ...
  function baseRest$1(func, start) {
    return setToString$1(overRest$1(func, start, identity$8), func + '');
  }
  ...
  var getMetricScale = function getMetricScale(state, data) {
    ...
    var yExtent = extent$1(data, _getMetricVal);
  ...
  function brush$1(dim) {
    ...
    brush.extent = function (_) {
      return arguments.length ? (extent = typeof _ === "function" ? _ : constant$c([[+_[0][0], +_[0][1]], [+_[1][0], +_[1][1]]]), brush) : extent;
    };

    brush.filter = function (_) {
      return arguments.length ? (filter = typeof _ === "function" ? _ : constant$c(!!_), brush) : filter;
    };
  ...
  var drawMiniSvg = function drawMiniSvg(state) {
    ...
    var miniYScale = linear$1().domain(extent$1(_data, _getMetricVal)).range([0, _options.plots.miniBarWidth]);
